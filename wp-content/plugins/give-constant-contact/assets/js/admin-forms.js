...
(function ( $ ) {
	 ...
	 *  @since: 1.0
	 */
	var toggle_constant_contact_fields = function () {

		var cc_enable_option = $( '.give-constant-contact-enable' );
		var cc_disable_option = $( '.give-constant-contact-disable' );
		...
		cc_enable_option.on( 'change', function () {
			...
			if ( cc_enable_option_val === false ) {
				$( '.give-constant-contact-field-wrap' ).slideUp('fast');
			} else {
				$( '.give-constant-contact-field-wrap' ).slideDown('fast');
			}
	...
	$( function () {
		...
		toggle_constant_contact_fields();
