	...
	Give_Fee_Recovery = {
		...
		give_fee_update: function( form, check_option, give_total, gateway ) {
			...
			if ( is_fee_recovery ) {
				...
				fee = give_fee_calculate( percentage, give_fee_unformat_amount( base_amount, form_decimal_separator ), give_fee_unformat_amount( give_total, form_decimal_separator ), give_fee_disable );
				...
				form.prepend( '<input type="hidden" name="give-fee-amount" value="' + give_fee_unformat_amount( give_fee_format_amount( fee, form ), form_decimal_separator ) + '"/>' );
			} else {
				form.find( 'input[name="give-fee-status"]' ).remove();
				form.prepend( '<input type="hidden" name="give-fee-status" value="disabled"/>' );
			}// End if().
