...
class Give_Fee_Recovery_Admin {
	...
	public function __construct() {
		...
		add_action( 'give_pre_process_give_forms_meta', array( $this, 'per_form_save' ), 10, 1 );
		...
		// Update Form meta value before save.
		add_filter( 'give_pre_save_form_meta_value', array( $this, 'pre_save_form_meta' ), 10, 2 );
	...
	public function gateway_field_save() {
		...
		foreach ( $_POST as $option_name => $option_value ) {
			...
			if ( false !== strpos( $option_name, 'give_fee_gateway_fee' ) ) {
				...
				if ( ( false !== strpos( $option_name, 'give_fee_gateway_fee_percentage' ) ) || ( false !== strpos( $option_name, 'give_fee_gateway_fee_base_amount' ) ) ) {
					$option_value = ! empty( $option_value ) ? give_fee_number_format( $option_value ) : '0.00';
				}
				// Update option.
				give_update_option( $option_name, $option_value );
			}

			if ( ( false !== strpos( $option_name, 'give_fee_percentage' ) ) || ( false !== strpos( $option_name, 'give_fee_base_amount' ) ) ) {
				$option_value = ! empty( $option_value ) ? give_fee_number_format( $option_value ) : '0.00';
				// Update option.
				give_update_option( $option_name, $option_value );
			}
	...
	public function percentage_text_settings( $value, $option_value ) {
		...
		if ( isset( $data_type ) ) {
			...
			$option_value                  = ( ! empty( $option_value ) ? give_format_decimal( $option_value ) : $option_value );
	...
	public function base_amount_text_settings( $value, $option_value ) {
		...
		switch ( $data_type ) {
			case 'price':
				$option_value          = ( ! empty( $option_value ) ? give_fee_format_amount( $option_value ) : $option_value );
	...
	public function per_form_save( $post_id ) {
		...
		foreach ( $_POST as $option_key => $option_value ) {
			...
			if ( false !== strpos( $option_key, '_form_gateway_fee_' ) ) {
				...
				if ( ( false !== strpos( $option_key, '_form_gateway_fee_percentage_' ) ) || ( false !== strpos( $option_key, '_form_gateway_fee_base_amount_' ) ) ) {
					$form_meta_value = ! empty( $form_meta_value ) ? give_fee_number_format( $form_meta_value ) : 0.00;
				}
	...
	public function donation_detail( $payment_id ) {
		...
		$total_donation = give_fee_format_amount( give_get_meta( $payment_id, '_give_payment_total', true ) );
		// Get donation amount.
		$donation_amount = give_fee_format_amount( give_get_meta( $payment_id, '_give_fee_donation_amount', true ) );
		...
		if ( ! empty( $subscription_id ) && $this->is_recurring_active() ) {
			...
			$donation_amount   = give_fee_format_amount( give_get_meta( $parent_payment_id, '_give_fee_donation_amount', true ) );
			$give_fee_amount   = give_fee_format_amount( give_get_meta( $parent_payment_id, '_give_fee_amount', true ) );

		} else {
			// Get Fee amount.
			$give_fee_amount = give_fee_format_amount( give_get_meta( $payment_id, '_give_fee_amount', true ) );
		}
		...
		?>
			<p>
				...
				if ( isset( $give_fee_amount ) && give_maybe_sanitize_amount( $give_fee_amount ) > 0 ) {
					$give_fee_amount = give_maybe_sanitize_amount( $give_fee_amount );
	...
	public function column_data( $value, $payment_id, $column_name ) {
		...
		if ( 'givefeerecovery' === $column_name ) {
			$fee             = give_fee_format_amount( give_get_meta( $payment_id, '_give_fee_amount', true ) );
	 ...
	 * @return string
	 */
	function fee_amount_string( $payment_id ) {
		...
		// Get Give Total donation amount.
		$give_amount = give_currency_filter( give_fee_format_amount( $payment_total ), array( 'currency_code' => $currency_code ) );

		// Check if Give fee amount is not empty.
		if ( ! empty( $give_fee_amount ) ) {

			// Get donation fee amount.
			$donation_amount = give_currency_filter( give_fee_format_amount( give_get_meta( $payment_id, '_give_fee_donation_amount', true ) ), array( 'currency_code' => $currency_code ) );
			$give_fee_amount = give_currency_filter( give_fee_format_amount( $give_fee_amount ), array( 'currency_code' => $currency_code ) );

			$give_amount = sprintf( __( '%1$s (%2$s donation + %3$s for fees)', 'give-fee-recovery' ), $give_amount, $donation_amount, $give_fee_amount );
		}
	...
	function after_updated_edited_donation( $payment_id ) {
		$payment_total   = give_get_meta( $payment_id, '_give_payment_total', true );
		$new_total       = ! empty( $_POST['give-payment-total'] ) ? give_fee_number_format( $_POST['give-payment-total'] ) : $payment_total;
		$new_total       = is_numeric( $new_total ) ? $new_total : 0;
		$give_fee_amount = give_get_meta( $payment_id, '_give_fee_amount', true );
		$give_fee_amount = ! empty( $give_fee_amount ) ? give_fee_number_format( $give_fee_amount ) : 0;

		// Get Give payment Fee amount.
		$new_fee_amount = ! empty( $_POST['give-payment-fee-amount'] ) ? give_fee_number_format( $_POST['give-payment-fee-amount'] ) : 0;
	...
	public function parse_report_dates( $data ) {
		...
		wp_redirect( add_query_arg( $dates, admin_url( 'edit.php?post_type=give_forms&page=give-reports&tab=' . esc_attr( $tab ) . $section_param . '&view=' . esc_attr( $view ) . '&form-id=' . absint( $id ) ) ) );
		give_die();
	}
	...
	/**
	 * Update Fee Percentage value before save.
	 *
	 * @param string $form_meta_value Form meta value.
	 * @param string $form_meta_key   Form meta key.
	 *
	 * @return string $form_meta_value
	 */
	public function pre_save_form_meta( $form_meta_value, $form_meta_key ) {

		if ( '_form_give_fee_percentage' === $form_meta_key ) {
			$form_meta_value = ! empty( $form_meta_value ) ? give_fee_number_format( $form_meta_value ) : 0.00;
		}

		return $form_meta_value;
	}
	...
	public function pre_save_global_value( $value, $option ) {
		...
		if ( ( false !== strpos( $option['id'], 'give_fee_percentage' ) ) || ( false !== strpos( $option['id'], 'give_fee_base_amount' ) ) ) {
			$value = ! empty( $value ) ? give_fee_number_format( $value ) : '0.00';
			// Update option.
